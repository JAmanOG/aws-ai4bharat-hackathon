 ...
 */
async function listHealthPortals(category, search) {
 ...
 * Get portal by ID.
 */
async function getHealthPortal(portalId) {
  const result = await query('SELECT * FROM health_portals WHERE id = $1', [portalId]);
  return result.rows[0] || null;
}

/**
 * AI-powered eligibility check for government health schemes.
 */
...
function getRuleBasedEligibility(userInfo) {
  const eligible = [];
...
module.exports = { listHealthPortals, getHealthPortal, checkEligibility };
